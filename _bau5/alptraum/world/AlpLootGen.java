...
public class AlpLootGen 
{
	Random rand = new Random();
	private World world;
	private static boolean canGen;
	
	public AlpLootGen(World worldObj)
	{
		world = worldObj;
		canGen = true;
	}
	...
	public boolean create(int i, int j, int k) 
	{
		...
		for(int a = 0; a < 5; a++)
		{
			...
			if(a == 4)
			{
				...
				int l = 5 + rand.nextInt(5);
				tec.setInventorySlotContents(h, new ItemStack(Alptraum.shiftingResidue, f+g +10));
				if(h != l)
				{
					tec.setInventorySlotContents(l, new ItemStack(Alptraum.blockUtility, 1, 0));
				}
				else tec.setInventorySlotContents(l + 1, new ItemStack(Alptraum.blockUtility, 1, 0));
